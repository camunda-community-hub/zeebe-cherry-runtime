...
public abstract class AbstractRunner {
    ...
    public FileVariable getInputFileVariableValue(String parameterName, final ActivatedJob activatedJob) throws ZeebeBpmnError {
        ...
        try {
            ...
            return fileVariableFactory.getFileVariable(fileVariableReference);
        } catch (Exception e) {
            throw new ZeebeBpmnError(BPMNERROR_ACCESS_FILEVARIABLE, "Worker [" + getName() + "] error during access parameterName[" + parameterName + "] :" + e);
        }
    }
    ...
    /**
     * This information is a code, used in any REST API.
     * If this information is not provide, the REST API will use the type.
     * See getIdentification() to see the rule
     */
    ...
    /**
     * To display on the admin page, this information is used.
     * If this information is not provide, then the name is used. Else the type
     * See getDisplayLabel()
     */
    ...
    private List<String> checkListParameters(List<RunnerParameter> listParameters) {
        ...
        for (RunnerParameter parameter : listParameters) {
            ...
            if (listParameters.stream().filter(w -> parameter.name.equals(w.getName())).count() != 1)
                listOfErrors.add("Runner [" + getIdentification() + ", Parameter name[" + parameter.name + "] is defined multiple time");

            if (parameter.conditionProperty != null) {
                if (parameter.conditionProperty.trim().isEmpty())
                    listOfErrors.add("Runner [" + getIdentification() + ", Parameter [" + parameter.name + "], conditionProperty must not bu empty when set");
                // the list must not be null
                if (parameter.conditionOneOf == null || parameter.conditionOneOf.isEmpty())
                    listOfErrors.add("Runner [" + getIdentification() + ", Parameter [" + parameter.name + "], conditionOneOf must not be empty");
                // the property must match a parameter
                if (listInput.stream().filter(w -> parameter.conditionProperty.equals(w.getName())).count() != 1)
                    listOfErrors.add("Runner [" + getIdentification() + ", Parameter [" + parameter.name + "], conditionProperty must match one and only one other parameter");
            }
        }
        return listOfErrors;
    }
